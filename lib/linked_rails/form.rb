...
require_relative 'form/option'
require_relative 'form/step'
    ...
    class << self
      ...
      def form_options(attr, opts)
        opts[:options].map do |key, values|
          LinkedRails::FormOption.new({key: key, attr: attr, klass: model_class, type: opts[:type]}.merge(values))
        end
      end
      ...
      def node_property_attrs(attr, attrs) # rubocop:disable Metrics/AbcSize
        ...
        klass_name = model_class.try(:reflections).try(:[], name.to_s)&.class&.name || name.to_s.classify
