...
describe LinkedRails::Collection do
  ...
  let(:scope) { {admin: true} }
  let(:type) { nil }

  before do
    LinkedRails.scope_strategy = ->(association, _user_context) { association }
  end
  ...
  describe '#apply_scope' do
    before do
      LinkedRails.scope_strategy = lambda { |association, user_context|
        user_context[:admin] ? association : Record.where(admin: false)
      }
    end
