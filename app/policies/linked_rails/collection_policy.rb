...
module LinkedRails
  class CollectionPolicy < LinkedRails.policy_parent_class
    def create_child?
      if parent_policy
        parent_policy.create_child?(record.association_class.name, collection: record, user_context: user_context)
      else
    ...
    def show?
      if parent_policy
        parent_policy&.index_children?(record.association_class.name, collection: record, user_context: user_context)
      else
